...
saveLzLinkEncoder : SaveLzLink -> Json.Encode.Value
saveLzLinkEncoder struct =
    Json.Encode.object
        [ ( "from", (zkNoteIdEncoder) struct.from )
        , ( "to", (zkNoteIdEncoder) struct.to )
        , ( "delete", (Maybe.withDefault Json.Encode.null << Maybe.map (Json.Encode.bool)) struct.delete )
...
saveZkNoteAndLinksEncoder struct =
    Json.Encode.object
        ...
        , ( "lzlinks", (Json.Encode.list (saveLzLinkEncoder)) struct.lzlinks )
        ]
...
type alias PrivateClosureRequest =
    { closureId : Maybe (Int)
    , request : PrivateRequest
    }


privateClosureRequestEncoder : PrivateClosureRequest -> Json.Encode.Value
privateClosureRequestEncoder struct =
    Json.Encode.object
        [ ( "closure_id", (Maybe.withDefault Json.Encode.null << Maybe.map (Json.Encode.int)) struct.closureId )
        , ( "request", (privateRequestEncoder) struct.request )
        ]


type alias PrivateClosureReply =
    { closureId : Maybe (Int)
    , reply : PrivateReply
    }


privateClosureReplyEncoder : PrivateClosureReply -> Json.Encode.Value
privateClosureReplyEncoder struct =
    Json.Encode.object
        [ ( "closure_id", (Maybe.withDefault Json.Encode.null << Maybe.map (Json.Encode.int)) struct.closureId )
        , ( "reply", (privateReplyEncoder) struct.reply )
        ]
...
saveLzLinkDecoder : Json.Decode.Decoder SaveLzLink
saveLzLinkDecoder =
    Json.Decode.succeed SaveLzLink
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "from" (zkNoteIdDecoder)))
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "to" (zkNoteIdDecoder)))
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "delete" (Json.Decode.nullable (Json.Decode.bool))))
...
saveZkNoteAndLinksDecoder =
    Json.Decode.succeed SaveZkNoteAndLinks
        ...
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "lzlinks" (Json.Decode.list (saveLzLinkDecoder))))
...
privateClosureRequestDecoder : Json.Decode.Decoder PrivateClosureRequest
privateClosureRequestDecoder =
    Json.Decode.succeed PrivateClosureRequest
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "closure_id" (Json.Decode.nullable (Json.Decode.int))))
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "request" (privateRequestDecoder)))


privateClosureReplyDecoder : Json.Decode.Decoder PrivateClosureReply
privateClosureReplyDecoder =
    Json.Decode.succeed PrivateClosureReply
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "closure_id" (Json.Decode.nullable (Json.Decode.int))))
        |> Json.Decode.andThen (\x -> Json.Decode.map x (Json.Decode.field "reply" (privateReplyDecoder)))
...
tauriReplyDecoder : Json.Decode.Decoder TauriReply
tauriReplyDecoder = 
    Json.Decode.oneOf
